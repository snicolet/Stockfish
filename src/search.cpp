...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    Piece movedPiece;
    int moveCount, captureCount, quietCount, pruningSafety;
    ...
    pruningSafety = 20 - ss->distanceFromPv;
    ...
    if (   !PvNode
        &&  depth < 9
        &&  eval - futility_margin(depth, improving) - pruningSafety >= beta
    ...
    if (   !PvNode
        ...
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 24 * depth - 34 * improving + 162 * ss->ttPv + 159 + pruningSafety
        && !excludedMove
    ...
    probCutBeta = beta + 209 - 44 * improving + pruningSafety;
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          else
          {
              ...
              if (   lmrDepth < 7
                  && !ss->inCheck
                  && ss->staticEval + 174 + 157 * lmrDepth + pruningSafety <= alpha
      ...
      if (    depth >= 7
       ...
       /* &&  ttValue != VALUE_NONE Already implicit in the next condition */
          ...
          &&  tte->depth() >= depth - 3)
      {
          Value singularBeta = ttValue - ((formerPv + 4) * depth) / 2 - pruningSafety;
