...
namespace {
  ...
  Depth reduction(bool improving, Depth depth, int moveNumber, int rangeCount) {
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          int lmrDepth = std::max(newDepth - reduction(improving, depth, moveCount, rangeCount), 0);
      ...
      {
          Depth r = reduction(improving, depth, moveCount, rangeCount);
