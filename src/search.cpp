...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   !PvNode
        &&  depth < 9
        &&  eval - futility_margin(depth, improving) - pruning_safety<BETA>(ss->ply, depth) >= beta
    ...
    if (   !PvNode
        ...
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 24 * depth - 34 * improving + 162 * ss->ttPv + 159 + pruning_safety<BETA>(ss->ply, depth)
        && !excludedMove
    ...
    probCutBeta = beta + 209 - 44 * improving + pruning_safety<BETA>(ss->ply, depth);
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          else
          {
              ...
              if (   lmrDepth < 7
                  && !ss->inCheck
                  && ss->staticEval + 174 + 157 * lmrDepth + pruning_safety<ALPHA>(ss->ply, depth) <= alpha
      ...
      if (    depth >= 7
       ...
       /* &&  ttValue != VALUE_NONE Already implicit in the next condition */
          ...
          &&  tte->depth() >= depth - 3)
      {
          Value singularBeta = ttValue - ((formerPv + 4) * depth) / 2 - pruning_safety<ALPHA>(ss->ply, depth);
