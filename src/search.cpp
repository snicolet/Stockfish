...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      {
          // bool inCheckPV = ss->inCheck && PvNode;
          ...
          else
          {
              ...
              if (   !givesCheck
                  && lmrDepth < 1
                  && captureHistory[movedPiece][to_sq(move)][type_of(pos.piece_on(to_sq(move)))] < -32 * depth * depth)
                  continue;
              ...
              // See based pruning
              if (!pos.see_ge(move, Value(-32) * depth * depth)) // (~25 Elo)
                  continue;
          }
      }
