...
Value MovePicker::see_sign(Move m) {

    // Use the SEE sign of the last move generated by MovePicker if we 
    // happen to know it, otherwise recalculate it.
    Value result = Value(move.see) != VALUE_NONE ? Value(move.see) : pos.see_sign(m);

    assert(m == move.move);   // Check that m is the last move generated
    assert(result == pos.see_sign(m));
