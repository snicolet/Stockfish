...
MovePicker::MovePicker(const Position& p, Move ttm, Depth d, const ButterflyHistory* mh,
                       const CapturePieceToHistory* cph, const PieceToHistory** ch, Bitboard rs)
           : pos(p), mainHistory(mh), captureHistory(cph), continuationHistory(ch), recaptureSquares(rs), depth(d) {

  assert(d <= DEPTH_ZERO);

  stage = pos.checkers() ? EVASION_TT : QSEARCH_TT;
  ttMove =   ttm
          && (depth > DEPTH_QS_RECAPTURES || (recaptureSquares & to_sq(ttm)))
