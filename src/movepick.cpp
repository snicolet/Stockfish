...
MovePicker::MovePicker(const Position& p, Move ttm, Depth d, const ButterflyHistory* mh,
                       const CapturePieceToHistory* cph, const PieceFromHistory** gh, const PieceToHistory** ch, 
                       Move cm, Move* killers)
           : pos(p), mainHistory(mh), captureHistory(cph), continuationHistory(ch), guardHistory(gh),
  ...
  for (auto& m : *this)
      if (Type == CAPTURES)
          ...
          m.value =  (*mainHistory)[pos.side_to_move()][from_to(m)]
                   + (*guardHistory[0])[pos.moved_piece(m)][to_sq(m)] / 2
                   + (*guardHistory[1])[pos.moved_piece(m)][to_sq(m)] / 2
