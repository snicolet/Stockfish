...
namespace {
  ...
  Score Evaluation<T>::evaluate_initiative(Score s) {
    ...
    // if (pos.pieces(QUEEN)) dbg_mean_of(flexibility);

    // Compute the initiative bonus

    initiative_mg = StockfishIsAttacking ? flexibility : flexibility;
    //initiative_mg = StockfishIsAttacking ? pieces + flexibility - 5 : 0;
    //initiative_mg = StockfishIsAttacking ? pieces + 5 - mobDiff : 0;
    //initiative_mg = pieces + 5 - mobDiff;
