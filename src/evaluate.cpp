...
namespace {
  ...
  // i.e., second order bonus/malus based on the known attacking/defending status
  // of the players.
  Score evaluate_initiative(const Position& pos, const EvalInfo& ei, Score score) {
    ...
    int openFiles = ei.pi->open_files();

    // Compute the initiative bonus for the attacking side
    int mg_initiative = -50 * openFiles;
    int eg_initiative = 8 * (asymmetry + kingDistance - 15) + 12 * pawns;

    // Now apply the bonus: note that we find the attacking side by extracting
    // the sign of the mg/eg values, and that we carefully cap the bonus so
    // that the score will never be divided by more than two.
