...
namespace {
  ...
  Score evaluate_initiative(const Position& pos, int asymmetry, int pawnTempi, Value eg) {

    int kingDistance = distance<File>(pos.square<KING>(WHITE), pos.square<KING>(BLACK));
    int pawns = pos.count<PAWN>(WHITE) + pos.count<PAWN>(BLACK);

    // Compute the initiative bonus for the attacking side
    //int initiative = 8 * (pawns + kingDistance - 17) + 12 * asymmetry + pawnTempi / 2 - 4;
    int initiative = 8 * (asymmetry + kingDistance - 17) + 12 * pawns;
    //dbg_mean_of(initiative);
...
Value Eval::evaluate(const Position& pos) {
  ...
  score += evaluate_initiative(pos, ei.pi->pawn_asymmetry(), ei.pi->pawn_tempi(), eg_value(score));
