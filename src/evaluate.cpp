...
namespace {
  ...
  Score evaluate_initiative(const Position& pos, const EvalInfo& ei, Value eg) {
    ...
    Bitboard b = (ei.pi->semiopenFiles[WHITE] | ei.pi->semiopenFiles[WHITE]) ^ 0xFF;
    int pawn_span = b ? int(msb(b) - lsb(b)) : 0;

    // Compute the initiative bonus for the attacking side
    int initiative = 8 * (asymmetry + outflanking - 16) + 12 * pawns + 2 * pawn_span;
