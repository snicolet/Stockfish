...
namespace {
  ...
  Score evaluate_initiative(const Position& pos, int asymmetry, Score score) {
    ...
    // Compute the endgame initiative bonus for the attacking side
    int initiative = 8 * (asymmetry + kingDistance - 15) + 12 * pawns;

    // Now apply the bonus: note that we find the attacking side by extracting
    // the sign of the midgame/endgame value, and that we carefully cap the bonus
    // so that the endgame score will never be divided by more than two.
    int bonus_mg = ((mg > 30) - (mg < -30)) * (-20);
