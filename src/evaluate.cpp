...
namespace {
  ...
  // is_simple_stalemate() detects two simple stalemates when the side to move has
  // no pieces except king or pawns: stalemated king, or stalemated king + blocked pawns.
  bool is_simple_stalemate(const Position& pos, const EvalInfo& ei) {

    const Color stm = pos.side_to_move();

    if (!(ei.attackedBy[stm][KING] & ~ei.attackedBy[~stm][ALL_PIECES]))  
    {
        // We know that the king can't move...
  ...
  Value do_evaluate(const Position& pos) {
    ...
    if (!pos.non_pawn_material(pos.side_to_move()) && is_simple_stalemate(pos, ei))
