...
namespace {
  ...
  Score evaluate_initiative(const Position& pos, const EvalInfo& ei, const Score positionnal_score) {

    int pawns     =  pos.count<PAWN>(WHITE)  + pos.count<PAWN>(BLACK);
    int asymmetry = ei.pi->semiopenFiles[WHITE] ^ ei.pi->semiopenFiles[BLACK];
  
    // Compute the initiative bonus for the attacking side
    int attacker_bonus =    7 * pawns - 98
                         + 10 * popcount<Max15>(asymmetry) ;
