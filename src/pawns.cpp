...
namespace {
  ...
  Score evaluate(const Position& pos, Pawns::Entry* e) {
    ...
    e->passedPawns[Us] = e->pawnAttacksSpan[Us] = e->weak[Us] = 0;
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        if (support | phalanx)
        {
            ...
            score -= Isolated + WeakUnopposed * int(!opposed) , e->weak[Us] |= SquareBB[s];

        else if (backward)
            score -= Backward + WeakUnopposed * int(!opposed) , e->weak[Us] |= SquareBB[s];
