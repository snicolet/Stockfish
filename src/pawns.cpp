...
Score Entry::do_king_safety(const Position& pos, Square ksq) {
  ...
  int advancedPawnDistance = 0;

  Bitboard pawns = pos.pieces(Us, PAWN);
  if (pawns)
  {
      advancedPawnDistance = distance<Rank>(ksq , frontmost_sq(Us, pawns));
      while (!(DistanceRingBB[ksq][minKingPawnDistance++] & pawns)) {}
  }

  if (relative_rank(Us, ksq) > RANK_4)
      return make_score(0, -16 * (minKingPawnDistance + advancedPawnDistance));
  ...
  return make_score(bonus, -16 * (minKingPawnDistance + advancedPawnDistance));
}
